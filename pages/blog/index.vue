...
export default {
  ...
  mounted () {
    ...
    let page = this.page
    this.$store.dispatch('ArticleList', { page }).then((response) => {
  ...
  methods: {
    ...
    toHome (id) {
      this.$router.push({ name: 'blog-tags', params: { tag_id: id } })
    },
    toArticle (id) {
      this.$router.push({ name: 'blog-article-id', params: { 'id': id } })
    },
    toArchives (id) {
      this.$router.push({ name: 'blog-archives', params: { 'tag_id': id } })
    },
    loadMore () {
